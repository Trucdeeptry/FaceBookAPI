...
async function seedNotifications() {
    ...
    try {
        ...
        const users = await usersCollection.aggregate([{ $sample: { size: 95 } }]).toArray();
        const post = await postsCollection.aggregate([{ $sample: { size: 95 } }]).toArray();
        const comment = await commentsCollection.aggregate([{ $sample: { size: 70 } }]).toArray();
