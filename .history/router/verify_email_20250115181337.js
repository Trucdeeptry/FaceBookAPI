...
function createVerificationToken(email) {
  client.get(email, (err, reply) => {
    if (err) {
      ...
      if (reply) {
        client.del(email, (delErr) => {
          if (delErr) {
            reject({
              message: "Lỗi khi xoá token cũ trong Redis",
              error: delErr,
            });
          } else {
            console.log("Xoá token cũ thành công");
          }
        });
      } else {
        // nếu không tạo tokken mới
        const token = jwt.sign({ email }, process.env.SECRET_KEY, {
          expiresIn: "1h",
        });
        client.setex(email, 900, token, (setErr) => {
          if (setErr) {
            reject({ message: "Lỗi lưu token vào Redis", error: setErr });
          } else {
            resolve(token); // Trả về token mới
          }
        });
      }
    }
