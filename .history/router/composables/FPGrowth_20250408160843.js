...
async function getAllHashtags() {
  try {
    const posts = await postsModel.find({}, { hashtags: 1, _id: 0 }); // chỉ lấy trường "hashtag", không lấy _id
    console.log(posts);
    
    const hashtags = posts.map((post) => post.hashtags:); // lấy mảng hashtag từ từng bài viết
    console.log(hashtags); // hashtags là mảng 2 chiều: [[...], [...], ...]
    return hashtags;
  } catch (err) {
