...
async function populateAuthorInfo(comment) {
    // Nếu thêm author_info(ko có trong schema) thì sẽ ko thêm được
    // Nên chuyển sang object rồi thêm
    let plainComment = comment.toObject();

  // Nếu chưa có author_info, fetch từ usersCollection
  if (!plainComment.author_info) {
    plainComment.author_info = await userModel.findOne(
      { _id: comment.user_id },
      { avatar: 1, info: 1 }
    );
  }
  ...
  if (plainComment.replies && plainComment.replies.length > 0) {
    plainComment.replies = await Promise.all(
      plainComment.replies.map((reply) => populateAuthorInfo(reply))
    );
  }
 if(comment._id == '67b6e3353decbde2f4057bac'){
    console.log(comment.author_info);
