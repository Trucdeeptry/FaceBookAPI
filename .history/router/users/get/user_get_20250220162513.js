...
import {getPostComments, getCommentsWithReplies} from '../../composables/comments'
...
router.get("/profile", async (req, res) => {
  try {
    ...
    const userPosts = await postsModel.aggregate([
      ...
      // Đưa mảng về object vì chỉ có 1 author
      {
        $unwind: {
        ...
        $addFields: {
          postComments: {
            $filter: {
              input: "$postComments",
              as: "comment",
              // Kiểm tra nếu object không rỗng: độ dài của mảng sau khi chuyển object thành mảng key-value > 0
