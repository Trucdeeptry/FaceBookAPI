...
router.post("/reg", async (req, res) => {
  try {
    ...
    if (!email || !password || !info) {
      return res
        .status(400)
        .json({ message: "email, password, info are required" });
    }
    const existingUser = await userModel.findOne({ email: email, status: 'success' });

    if (existingUser) {
      return res.status(400).json({ message: "Email already exists", status: 'success' });
    }
