...
// import crud token
const { isExistToken, isExistEmail } = require("./crud_token");

// Hàm tạo token xác nhận
function createVerificationToken(email) {
  const isExist = isExistEmail(email);
  if (isExist) {
    deleteEmail(email);
  }
...
router.post("/send-verification", async (req, res) => {
  const { email, message = Thank you for registering with TSocial. To complete your registration } = req.body;
  try {
...
router.get("/verify", (req, res) => {
  ...
  if (isExist) {
    ...
    return res
      .status(400)
      .json({ message: "Session are old. Please check the lastest email" });
  }
});
