        ...
        <body>
          <div class="email-container">
            ...
            <div class="email-body">
              ...
              <a href="${process.env.URL}/verify-status?email=${to}&token=${token}" class="button">Verify Your Email</a>
...
router.get("/verify", (req, res) => {
  ...
  if (!token) {
    return res.status(400).json({ message: "Something when wrong. Please contact the developer" });
  }
  jwt.verify(token, process.env.SECRET_KEY, (err, decoded) => {
    if (err) {
      return res
        .status(400)
        .json({ message: "Resgistraion session has ended. Please try again" });
    }
