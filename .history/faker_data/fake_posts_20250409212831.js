...
function getRandomHashtags(topic) {
  ...
  const count = randomIntFromInterval(1, hashtags.length - 14);
...
function getShuffelHashTag(selectedTopics) {
  const results = selectedTopics.flatMap((topic) => topics[topic].hashtags);
...
async function seedPosts() {
  ...
  try {
    ...
    let postsData = [];
    for (let i = 0; i < 50; i++) {
      console.log("Đang tạo bài viết thứ: ", i);
      const keys = Object.keys(topics);

      const primaryKey = keys[randomIntFromInterval(0, keys.length - 1)];
      const topic = topics[primaryKey];
      const selectedTopics = pickRelatedTopics(
        primaryKey,
        randomIntFromInterval(1,23)
      );
      const topicsHashtags = getShuffelHashTag(selectedTopics);
      const users = await usersCollection.find().toArray(); 
      const authorId = users[randomIntFromInterval(0, users.length - 1)]._id; 
      const likesCount = randomIntFromInterval(0, 8); 
      let post = {
        user_id: authorId,
        hashtags: topicsHashtags,
        content: topic.content, 
        image: await topic.image(), 
        liked_by: await generateLikes(likesCount, usersCollection), 
        created_at: faker.date.past(), 
      };
      postsData.push(post);
      console.log(`Tạo bài viết ${i} thành công`);
    }
