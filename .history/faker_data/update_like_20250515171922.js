...
async function updateLikesForUser() {
  const allPosts = [
    {
      hashtags: ["#tag1", "#tag2"],
    },
  ...
  const user_hashtags = ["#tag1", "#tag3"];
  const matchedPosts = allPosts.filter((post) => post.hashtags.some(tag => user_hashtags.includes(tag)));
