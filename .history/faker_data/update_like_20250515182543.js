  ...
  for (const post of matchedPosts) {
    ...
    if (!alreadyReacted) {
      ...
      try {
        ...
        if (!response.ok) {
          const text = await response.text(); // lấy nội dung trả về dạng text (dù không phải JSON)
          console.error("❌ Lỗi khi cập nhật likes text:", response.status, text);
          return;
        }
        ...
        console.log("❌ Lỗi khi cập nhật likes:", error.message);
        return;
      }
    }
  }
