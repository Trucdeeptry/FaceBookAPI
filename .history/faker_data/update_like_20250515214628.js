...
// }
async function updateLikesForUser(userId, userHashtags, allPosts) {
  const userHashtagsLower = normalizeHashtags(userHashtags);
  const matchedPosts = allPosts.slice(0, 2).filter((post) => {
    ...
    const postHashtagsLower = post.hashtags;
    console.log(postHashtagsLower, userHashtagsLower);
    ...
    if (isMatch) {
      console.log(`✅ [${userHashtagsLower}] Khớp hashtag:`, postHashtagsLower);
    }
    return isMatch;
  });
