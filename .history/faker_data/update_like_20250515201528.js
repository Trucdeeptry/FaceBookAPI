...
async function updateLikesForUser(userId, userHashtags, allPosts) {
  const matchedPosts = allPosts.filter((post) => {
    ...
    const postHashtagsLower = post.hashtags.map((tag) => tag.toLowerCase());
    const userHashtagsLower = userHashtags.map((tag) => tag.toLowerCase());

    const isUserMatched = userHashtagsLower.some((userTag) =>
