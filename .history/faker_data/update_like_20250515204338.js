...
async function updateLikesForUser(userId, userHashtags, allPosts) {
  const matchedPosts = allPosts.filter((post) => {
    ...
    const isMatch = userHashtagsLower.some((userTag) =>
      postHashtagsLower.some((postTag) => {
        if (userTag.length < minLength || postTag.length < minLength) {
          return false;
        }
        const distance = levenshteinDistance(userTag, postTag);
        const isRelated =
          distance <= Math.min(userTag.length, postTag.length) / 3;
        if (isRelated) {
          console.log(
            `✅ [${post.id}] Hashtag tương đồng: ${userTag} ~ ${postTag} (distance: ${distance})`
          );
        }
        return isRelated;
      })
