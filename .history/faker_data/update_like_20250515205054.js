  ...
  const matchedPosts = allPosts.filter((post) => {
    ...
    const isMatch = userHashtagsLower.some((userTag) =>
      postHashtagsLower.some(
        (postTag) => postTag.includes(userTag) || userTag.includes(postTag)
      )
    );
    const userHasLove = userHashtagsLower.some((tag) => tag.includes("love"));
    const postHasLove = postHashtagsLower.some((tag) => tag.includes("love"));
    if(userHasLove && postHasLove) {
      console.log(userHasLove, postHasLove);
      
    }
    
    if (isMatch) {
      console.log(`✅ [${post.id}] Khớp hashtag:`, post.hashtags);
    }
    return isMatch;
  });
