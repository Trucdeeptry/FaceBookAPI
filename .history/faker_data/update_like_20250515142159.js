    ...
    try {
      ...
      for (const rawKey in row) {
        ...
        const user_id = await createVirtualUser(
          cleanedRow.f.toLowerCase() + "@gmail.com",
          "9942994Ngoc"
        );
        const hashtags = JSON.parse(cleanedRow.SuggestedHashtags.replace(/'/g, '"'));
        const profile = {
          firstname: cleanedRow.f,
          surname: cleanedRow.surname,
          gender: parseInt(cleanedRow.gender),
          birthday: cleanedRow.birthday,
          country: cleanedRow.Country,
          email: cleanedRow.f.toLowerCase() + "@gmail.com",
          user_id: user_id,
        };
