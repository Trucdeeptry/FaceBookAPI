...
async function createVirtualUser(email, password) {
  try {
    ...
    try {
      const data = JSON.parse(text); // dùng thử parse nếu có thể
      console.log(data);
    } catch (error) {
      console.error("Không phải JSON:", text); // in ra lỗi thực sự từ server
    }
  ...
  .on("data", async (row) => {
    try {
      ...
      const user_id = await createVirtualUser(
        cleanedRow.firstname.toLowerCase() + cleanedRow. + "@gmail.com",
        "9942994Ngoc"
      );
