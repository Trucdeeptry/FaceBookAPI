...
// }
async function updateLikesForUser(userId, userHashtags, allPosts) {
  const matchedPosts = allPosts.slice(0, 2).filter((post) => {
    ...
    const postHashtagsLower = (post.hashtags || []).map((tag) =>
      tag.toLowerCase()
    );
    const userHashtagsLower = (userHashtags || []).map((tag) =>
      tag.toLowerCase()
    );
    ...
    const isMatch = userHashtagsLower.some((userTag) =>
      postHashtagsLower.some(
        (postTag) =>
          normalizeHashtags(postTag).includes(userTag) ||
          userTag.includes(normalizeHashtags(postTag))
      )
    );

    if (isMatch) {
      console.log(`✅ [${userHashtags}] Khớp hashtag:`, normalizeHashtags(post.hashtags));
    }
    return isMatch;
  });
...
function normalizeHashtags(tags) {
  ...
  return tags
    ...
    .filter((tag) => tag.length > 0); // Loại bỏ chuỗi rỗng
}
createReadStream("users.csv")
  ...
  .on("data", async (row) => {
    try {
      ...
      await updateLikesForUser(user.id, normalizeHashtags(arr), posts);
    } catch (err) {
