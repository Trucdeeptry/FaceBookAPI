...
// }
async function updateLikesForUser(userId, userHashtags, allPosts) {
  const matchedPosts = allPosts.slice(0,10).filter((post) => {
    ...
    const isMatch = userHashtagsLower.some((userTag) =>
      postHashtagsLower.some(
        (postTag) => normalizeHashtags(postTag).includes(userTag) || userTag.includes(normalizeHashtags(postTag))
      )
    );
    
    if (isMatch) {
      console.log(`✅ [${userHashtags}] Khớp hashtag:`, post.hashtags);
    }
    return isMatch;
  });
