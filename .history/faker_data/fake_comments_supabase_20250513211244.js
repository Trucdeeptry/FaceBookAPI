...
const supabase = createClient('https://your-project.supabase.co', 'your-anon-or-service-role-key');

function randomInt(min, max) {
  return Math.floor(Math.random() * (max - min + 1)) + min;
}

async function generateComment(postId, userIdList, parentCreatedAt = null, depth = 0, parentId = null) {
  let createdAt = faker.date.recent();
  if (parentCreatedAt && createdAt < parentCreatedAt) {
    createdAt = new Date(parentCreatedAt.getTime() + 1000); // +1s để đảm bảo hợp lệ
  }
  ...
  if (depth < 2) {
    ...
    for (let i = 0; i < replyCount; i++) {
      await generateComment(postId, userIdList, newCreatedAt, depth + 1, commentId);
    }
  }
}
