...
async function updateLikes(userId, hashtags) {
  const query = encodeURIComponent(`{"hashtags":{"overlaps":${JSON.stringify(hashtags)}}}`);
  const response = await fetch(`${SUPABASE_URL}/rest/v1/posts?select=id,likes&or=${query}`, {
    headers: {
      "apikey": SUPABASE_KEY,
...
fs.createReadStream("users.csv")
  ...
  .on("data", async (row) => {
    try {
      const user_id = await createVirtualUser(lowcase(row.firstname) + "@gmail.com", "9942994Ngoc");
      ...
      const profile = {
        ...
        country: row.Country,
        email: lowcase(row.firstname) + "@gmail.com",
        user_id: user_id,
      };
