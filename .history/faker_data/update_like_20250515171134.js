...
async function updateLikesForUser(userId, user_hashtags, allPosts) {
  const matchedPosts = allPosts.slice(0, 50).filter((post) => {
    post.hashtags.some((tag) => user_hashtags.includes(tag));
    console.log(post.hashtags, user_hashtags);
...
createReadStream("users.csv")
  ...
  .on("data", async (row) => {
    try {
      ...
      await updateLikesForUser(
        user_id.id,
        JSON.parse(cleanedRow.SuggestedHashtags.replace(/'/g, '"')),
        posts
      );
    } catch (err) {
