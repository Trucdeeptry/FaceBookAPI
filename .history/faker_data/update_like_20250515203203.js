...
createReadStream("users.csv")
  ...
  .on("data", async (row) => {
    try {
      ...
      const filePath = "user_hashtags.json";

      const oldData = JSON.parse(readFileSync("user_hashtags.json", "utf-8"));

      // Gộp mảng và loại bỏ trùng lặp (nếu cần)
      const merged = Array.from(new Set([...oldData, ...arr]));

      // Ghi lại toàn bộ mảng mới
      writeFileSync(
        "user_hashtags.json",
