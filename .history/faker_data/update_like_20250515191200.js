...
async function updateLikesForUser(userId, userHashtags, allPosts) {
  ...
  try {
    await fs.writeFile(
      "/mnt/data/debug_likes.json",
      JSON.stringify({ matchedPosts, userHashtags }, null, 2),
      "utf-8"
    );
    console.log("✅ Đã ghi debug_likes.json thành công");
  } catch (err) {
  ...
  for (const post of matchedPosts) {
    ...
    if (!alreadyReacted) {
      ...
      try {
        ...
        // Không return ở đây để tiếp tục xử lý các post khác
      }
    }
  }
}
